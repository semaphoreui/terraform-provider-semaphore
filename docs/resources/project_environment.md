...
- `value_wo` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) Write-only variable value. Change `value_wo_version` to rotate. Conflicts with `value`. Ensure that if an attribute is set, these are not set: "[<.value]". Ensure that if an attribute is set, also these are set: "[<.value_wo_version]".
