...
- `password_wo` (String, Sensitive) Login Password. Write-only version for ephemeral compatibility. Ensure that if an attribute is set, these are not set: "[password]". Ensure that if an attribute is set, also these are set: "[password_wo_version]".
